...
We store that data in redis and ensure service will be able to commit
"""
@router.post("/prepare", response_model=PrepareResponse)
async def prepare(request: UserPrepareRequest, db: AsyncSession = Depends(get_db)):
    logger.info(
    ...
    try:
        result = await purchases_service.prepare(request.transaction_id, request.user_id, request.amount, db)
        return result
...
"""
Endpoint searches data from KV-storage by the transaction id.
Purchase is being commited to the microservice db
"""
@router.post("/commit", response_model=UserCommitResponse)
async def commit(request: UserCommitRequest, db: AsyncSession = Depends(get_db)):
    logger.info(
